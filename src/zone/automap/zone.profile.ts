import { Mapper, MappingProfile, createMap, forMember, mapFrom, typeConverter } from "@automapper/core";
...
export class ZoneProfile extends AutomapperProfile {
    ...
    get profile(): MappingProfile {
        return (mapper) => {
            createMap(mapper, Zone, GetZoneDto, 
                // If contact has no @ sign for the email. Replace the first '.' we run into with the @
                forMember(
                    (destination) => destination.contact, 
                    mapFrom((source) => StringUtils.putAtBack(StringUtils.trimFromEnd(source.contact, '.')))
                ),
                forMember(
