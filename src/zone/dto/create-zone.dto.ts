...
export class CreateZoneDto {
    ...
    @ApiProperty({
        description:
            'The root host value for this zone.',
    })
    //@Transform((property) => property.value.lastIndexOf('.') == property.value.length ? property.value.substring(0, property.value.lastIndexOf('.')) : property.value )
    hostname: string
    ...
    @ApiProperty({
        description:
            'The email address of the contact responsible for this zone',
    })
    contact: string
    ...
    @ApiProperty({
        description:
            'When this SOA record in seconds expires',
    })
    expiry: number
}
