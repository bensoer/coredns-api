...
describe('RecordController', () => {
  ...
  it('should create a record', async () => {
    ...
    jest.spyOn(recordService, 'create').mockImplementation(async (zoneGuid: string, createRecordDto: CreateRecordDto) => record)
    ...
    expect(recordService.create).toHaveBeenCalledWith(zoneGuid, createRecordDto)
  });
  ...
  it('should get all records of a zone', async () => {
    ...
    const recordsAsGetRecords = records.map((record) => {return {guid: record.guid, domain: record.domain, type: record.type, content: record.content} as GetRecordDto })
    
    jest.spyOn(recordService, 'findAll').mockImplementation(async (zoneGuid: string) => records)
  ...
  it('should get a record', async () => {
    ...
    const recordAsGetRecordDto = {guid: record.guid, domain: record.domain, type: record.type, content: record.content} as GetRecordDto

    jest.spyOn(recordService, 'findOne').mockImplementation(async (recordGuid: string) => record)
  ...
  it('should update a record', async () => {
    ...
    jest.spyOn(recordService, 'update').mockImplementation(async (recordGuid: string, updateRecordDto: UpdateRecordDto) => record)

    const updatedRecord = await controller.update(zone.guid, record.guid, updateRecordDto)
    ...
    expect(recordService.update).toHaveBeenCalledWith(record.guid, updateRecordDto)
  });
  ...
  it('should delete a record', async () => {
    ...
    jest.spyOn(recordService, 'remove').mockImplementation(async (recordGuid: string) => record)
