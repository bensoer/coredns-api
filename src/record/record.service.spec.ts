...
describe('RecordService', () => {
  ...
  it('insert should be successfull', async () => {
    ...
    jest
      ...
      .mockImplementation(async (guid: string) => zone);
    jest
      .spyOn(recordRepository, 'insert')
      .mockImplementation(async (record: Record) => 5);
  ...
  it('findAll should succeed', async () => {
    ...
    jest
      ...
      .mockImplementation(async (guid: string) => zone);
    jest
      .spyOn(recordRepository, 'readAllOfZone')
      .mockImplementation(async (zoneId: number) => {
        return { totalItems: records.length, entities: records };
      });
  ...
  it('findAll should 500 if readAllOfZone Repository Call Fails', async () => {
    ...
    jest
      .spyOn(recordRepository, 'readAllOfZone')
      .mockImplementation(async (zoneId: number) => {
        throw errorThrownByReadAllOfZone;
      });
    ...
    try {
      ...
      expect(error).toEqual(
        new InternalServerErrorException(errorThrownByReadAllOfZone),
      );
    }
  ...
  it('findOne should 404 if not found', async () => {
    ...
    jest
      .spyOn(recordRepository, 'read')
      .mockImplementation(async (recordGuid: string) => {
        throw new EntityNotFoundError(Record, undefined);
      });
  ...
  it('remove should succeed', async () => {
    ...
    jest
      ...
      .mockImplementation(async (recordGuid: string) => 1);
  ...
  it('remove should 404 if delete does not remove any records', async () => {
    ...
    jest
      .spyOn(recordRepository, 'delete')
      .mockImplementation(async (recordGuid: string) => 0);
